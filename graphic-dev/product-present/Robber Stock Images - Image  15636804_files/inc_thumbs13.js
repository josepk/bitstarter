    ...
    for (var i = 0; i < imgSizes.length; i++) {
      ...
      for (var j = 0; j < row.length; j++) {
        ...
        if ($.browser.msie && $.browser.version <= 7) {
          ...
          if (rid == "nextbutton") {
            $(sel + rid, $container).attr({'style': 'margin-bottom: ' + ((rowHeight - imgSizes[rij][2]) / 2) + 'px !important;'});
          } else {
            $(sel + "thb_cell" + rid, $container).attr({'style': 'width: ' + (riw - 2 + padding + dx) + 'px !important; height: ' + (rowHeight + 15) + 'px !important;'});
          }
        } else {
          if (rid == "nextbutton") {
            $(sel + rid, $container).attr({'style': 'margin-bottom: ' + ((rowHeight - imgSizes[rij][2]) / 2) + 'px !important;'});
          } else {
            $(sel + "thb_cell" + rid, $container).attr({'style': 'height: ' + (rowHeight + 15) + 'px !important;'});
          }
        }
      }
      row = [id];
...
$('div[id^="thb_cell"]').each(function() {
  var id = $(this).attr('id').replace('thb_cell','');
